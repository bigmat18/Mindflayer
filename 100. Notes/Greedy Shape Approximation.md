...
##### Algorithm Invariant
To guarantee a valid shape approximation we follow a **invariant** during the run of the algorithm:
...
##### Merge Priorities
For each adjacent pair $R_i$ and $R_j$ of regions, we could compute the [[Variational Shape Approximation|shape measure E(R', P')]] and order the region pairs by increasing shape error. In ordfer to speed up the algortithm, the exact $L²$ is approximated by:
$$
L²(f') = L²(D_i, P') + L²(D_j, P')
$$
since $D_i$ usually contains much fewer triangles than $R_i$, this will significantly [[Speedup#Speedup|speed up]] the algorithm. 
