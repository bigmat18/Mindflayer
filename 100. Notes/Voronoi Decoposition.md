...
The [[Refinement Introduction|dual]] of the Voronoi Diagram is a **nice** triangulation (called **[[Delaunay Triangulation]]**) of the point set. We connect each points adding an edge. Each triangle has the **empty-circle** property, if we do a circle that crosses the three points it not contains any other point, this means the triangulation is well done.
