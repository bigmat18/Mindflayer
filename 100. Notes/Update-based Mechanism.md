...
The copy is caused by the execution of **STORE S by P**. Furthermore, for memory ordering reasons this instruction must be **synchronous** (or we need a **[[Safety Nets (fences)|fence]]** before the notification). Memory ordering gurantees that **STORE S by P -> LOAD S by Q** and cache coherence guarantees that the data eventually is PE_Q's cache is updated.
