...
- The 5 decomposition is more simple and obvious, 3 regular tetrahedral and 1 irregular, the bad thing of this decomposition is **oriented**, that means if we take 2 cube and I put one next to the other the diagonal not match. This break the symmetric property of cube.
...
The core idea is, for each vertex generated by marching cube: generate **a patch** and **one quad** for each intersected edge (the 4 vertices associated to the patches of the cells sharing the edge). This approach tend to improve triangles quality but introduce complexity. 
