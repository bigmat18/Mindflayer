...
### Structural Hazards
It happens when two stages need the same hardware resources in the same cycle. For example and if want read an instruction and, in the same time, another MEM want do a load/store.
- we can duplicate resources
- we can block one of the two sages

### Control Hazards
This happens when there is branch statement, in this case the pipeline could be blocked until the branch was done. We can fix this issue with mechanism of branch predicitoon.
