...
It can also computed using the [[Inter Calculation Time|inter-departure time]] or [[Processing Bandwidth & Throughput|bandwidth]]. We compute the execution time with parallelism 1 against the execution time with parallelism n by keeping the problem size fixed.
#### [[Amdahl's Law]]
...
The week scalability describe how much a parallel versione remains constant in terms of time with the increasing of problem size. 
