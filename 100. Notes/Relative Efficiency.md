...
$$
E(p) = \frac{S(p)}{p} = \frac{\frac{T_{t-seq}}{T_{c}(p)}}{p} \frac{T_{t-seq}}{T_c(p) \times p}
$$
$T_c(p) \times p$ is also called **cost of parallization** (or simply **cost**), $T_{t-seq}$ is the execution time in the sequential case, $T_{c}(p)$ is using p processing. 

If $T_c(p) \times p > T_{c-seq}$ then the parallelization overhead (is, communication, synchronization, idleness) impairs parallel efficiency and performance.
...
##### Efficiency and [[Processing Bandwidth & Throughput|Bandwidth]]
